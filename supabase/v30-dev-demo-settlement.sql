...
-- This version deliberately omits the category column so the database default
-- ('Inne') is used and no category constraint can be hit.
...
where not exists (
  ...
  where e.household_id = d.household_id
    and e.import_key = 'portfolio-demo-settlement-02'
);
